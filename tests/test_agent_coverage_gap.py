...
def test_salvage_tool_args_exec_command_garbled():
    # Targets lines 619-621 (success salvage)
    raw = 'command": "ls -la"}'
...
def test_run_session_log_dir_override(tmp_path):
    # Targets line 1167 (the IF branch)
    with patch('agent._llm_request', side_effect=Exception("stop")), \
         patch('agent._estimate_tools_tokens', return_value=100):
        try:
            agent.run_session("hello", log_dir_override=str(tmp_path / "logs"))
        except Exception:
            pass
