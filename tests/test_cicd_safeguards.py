...
def test_validate_tool_call_pr_create_blocked_no_closes():
    ...
    blocked, msg = _validate_tool_call("exec_command", {"command": "gh pr create --body 'Fixes bug'"}, True, mock_log, is_cicd_builder=True)
    assert blocked is True
...
def test_validate_tool_call_pr_create_allowed_with_closes():
    ...
    blocked, msg = _validate_tool_call("exec_command", {"command": "gh pr create --body 'Closes #123'"}, True, mock_log, is_cicd_builder=True)
    assert blocked is False
    assert msg is None
...
def test_validate_tool_call_pr_create_allowed_with_cat_file(tmp_path):
    ...
    with patch("builtins.open", mock_open(read_data="This PR Closes #456")):
        blocked, msg = _validate_tool_call("exec_command", {"command": "gh pr create --body \"$(cat /tmp/pr-body.md)\""}, True, mock_log, is_cicd_builder=True)
        assert blocked is False
        assert msg is None
