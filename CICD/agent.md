...
6. **Update the PR body to match the new diff** — the inherited body describes the OLD fix and is now misleading. Write the corrected body (with the Phase 8 Acceptance Criteria checklist + Regression Evidence) to `/tmp/pr-body-${ISSUE}.md` and run `python3 tools/gh_wrapper.py pr edit <N> --body "$(cat /tmp/pr-body-${ISSUE}.md)"` (preferred — auto-handles the deprecation warning AND verifies the body actually landed). The bare `gh pr edit <N> --body ...` still works but you must do the verify-after-write step yourself.

   **gh-deprecation quirk (run 1007 failure mode — turns 119-126 stuck loop, addressed by issue #1009 / `tools/gh_wrapper.py`):** `gh pr edit` and `gh pr view` often return exit code 1 with a "GraphQL: Projects (classic) is being deprecated ... (projectCards)" warning in stderr, **even when the operation succeeded**. Do NOT retry on this exit code. `tools/gh_wrapper.py` handles this by (a) treating exit=1+deprecation-warning as success for read subcommands, and (b) for `pr edit <N> --body <CONTENT>`, re-reading the body with `gh pr view <N> --json body --jq '.body'` and surfacing exit=1 if the body did NOT actually land (a real failure can also look like deprecation+exit=1 if you don't verify). If you call bare `gh pr edit` instead of the wrapper, verify the result with `gh pr view <N> --json body --jq '.body' | head -5` — if the body shows the new content, the edit worked; treat exit-1-with-deprecation-warning as success and proceed. Retrying burns turns and the body never changes because every attempt "fails" identically.
...
**Stale worktree HEAD — check origin before re-writing (cycle 103 — run 217 × 26 turns).** If `git status` shows only `.coverage` modified AND `git diff tests/` is empty AND you believe you wrote changes, do NOT immediately conclude your file write failed. First run: `git fetch origin && git log HEAD..origin/$(git branch --show-current) --oneline`. If origin has commits you haven't pulled, **`git pull` them**, re-run pytest, and verify whether the work is already done before re-writing anything. Run 217: builder spent 26 turns re-writing tests that were already correct on origin (beewatcher had pushed `1fe622d`) because its async summary was frozen at an older HEAD and it never fetched to compare. Silent file-write failure and stale-origin are two different diagnoses for the same symptom — check origin FIRST (one command), file-write failure second.

**`file.delete` DESTROYS THE ENTIRE FILE — never use it to remove lines (cycle 102 — run 211 destruction × 2).** `file(action='delete')` is a filesystem DELETE that removes the whole file unconditionally. The `start_line` and `end_line` parameters are **silently ignored**. To remove specific lines: (1) `file(action='read')` the complete file, (2) write the corrected full content with `file(action='write')`. NEVER pass `start_line`/`end_line` to `file(action='delete')`.

**Verify line count after every file modification (cycle 102).** After every `file append`, `file write`, `file insert`, or `file replace`: immediately run `wc -l <file>` and confirm the line count changed as expected. If the count is UNCHANGED after an `append` that claimed to add content, the append failed — do NOT run tests assuming the new content is there. Re-check the file path and retry.
...
**New ctx field that handlers dereference hard → AttributeError at every ctx-construction site (issue #1045 / PR #1046).** When a `commands.py::_cmd_*` handler reads a NEW ctx attribute directly (e.g. `ctx.set_model`, `ctx.summary_base_url`) instead of via `getattr(ctx, "x", default)`, EVERY path that builds its own ctx must supply that field or it raises `AttributeError` at runtime — including test fixtures. Before opening the PR: (1) add the new field to `_make_ctx` in `tests/test_commands.py`; (2) `grep -nE "pick_model=|SimpleNamespace\(" agent.py` to confirm there is exactly ONE production ctx-construction site and update it; (3) run `tests/test_agent_loop_commands.py` IN ADDITION to `tests/test_commands.py` — it exercises the loop's real ctx and catches a missing field that the curated subset hides. (The codebase rule "handlers short, free of business logic" is the mitigation: pass a single callback like `set_model(role, id)` that bundles config+backend+persist, so the handler stays thin and the logic is unit-testable in `agent.py`.)

**Config-writing / persistence tests must `chdir` to a tmpdir (issue #1045 / PR #1046).** Helpers that write to `os.getcwd()/.agent/config.json` (e.g. `_persist_config_value`) will pollute the worktree and make the test non-hermetic when run from the repo root. Use `monkeypatch.chdir(tmp_path)` (or `os.chdir` + restore in a fixture) so the write lands in an isolated dir, and assert against that tmp file — never the real `.agent/config.json`.
...
**Repair cascade recovery (general).** If you have made 3 or more attempts to fix the same file and `py_compile` / `pytest` still reports errors, STOP iterating on that file. Run `git checkout HEAD -- <file>` to restore the original, then re-implement from scratch using `insert` with explicit line numbers (found via `search_files`) rather than `append`. Do not continue patching a file that has been broken by multiple failed edits — each patch attempt shifts the error location without fixing the root cause. If after a fresh start the file still fails after 3 attempts, comment on the issue and move to a different target this cycle.
...
**Test-failure plateau rule (cycle 101 — run 210 null cause).** If the **same test name(s)** appear in pytest FAILED output across **3 consecutive runs** (regardless of error type — NameError, KeyError, ValueError, AttributeError all count), **stop patching the mock setup**. The target function requires too much internal mocking to converge this way. Instead: (1) revert the failing test additions (`git checkout HEAD -- tests/<file>.py` to restore the last-passing state), (2) verify the original tests still pass, (3) proceed immediately to TRACK and open the PR with the original passing baseline. Document the attempted test approach and the 3-run failure in the PR body so the reviewer understands. This rule fires even if you believe "one more fix" will work — the 3-failure signal means it won't, and the budget is better spent on a PR.
...
```
**Note on `gh pr edit` from this point on (issue #1009):** if you later need to fix the PR body (typo, missing AC), prefer `python3 tools/gh_wrapper.py pr edit <PR_NUM> --body "$(cat /tmp/pr-body-${ISSUE}.md)"` over bare `gh pr edit`. The wrapper auto-handles the Projects-classic deprecation warning AND verifies the body actually landed by re-reading via `gh pr view <PR_NUM> --json body`. With bare `gh pr edit`, an exit=1 caused by the deprecation warning can mask a real write failure — `gh_wrapper.py` distinguishes the two cases for you.
