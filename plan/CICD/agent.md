...
- Cycle 0033 P-count probe: the agent called `search_files` first (got 100 results truncated) then fell back to `exec_command` with `grep -c` — 2 tool calls for a counting task that needs 1. The `search_files` tool has no count-only mode; adding one (e.g. `count_only=True` parameter returning a single integer) would eliminate the fallback round-trip for count-oriented queries. Metric: tool-call count on P-count probe — baseline 2, target 1.
