...
4. VERIFY — run tests, re-measure metric, re-run probe, sweep diff hygiene
...
Read: `reviews.md`, `progress.md`, recent improvement plans.
...
```
If red, stop — file a `bug`+`regression`+`cicd` issue and defer all reviews.
...
```
All must pass. Compare count to PR's claimed before/after. Grep diff for new `skip`/`skipIf`/`skipUnless`.
...
**Step 3 — Probe re-run** against PR's `agent.py` in a fresh temp dir. Compare wall time, turns, tool calls to builder's baseline.
...
Append to `reviews.md`: `| R-NNN | date | #PR | #ISSUE | verdict | metric? | tests | reason |`
...
Create `reviews.md` with header table if missing. `mkdir -p /tmp/agent-cicd-review/`. Pick `R-NNN` by incrementing highest in `reviews.md`. Reviewer cycle numbers (`R-0001`) are independent of builder numbers (`0001`).

## Hard Rules

1. **Independent verification mandatory.** Re-run tests, re-measure metric, re-run probe from clean worktree.
